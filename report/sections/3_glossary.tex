...
\begin{itemize}
    \item \textbf{OPERAZIONE 1}: Ricercare disponibilità di un professore. Dato il nome e cognome di un docente e il periodo didattico, restituire il suo calendario settimanale delle lezioni (giorno, fascia oraria e aula). 
    \item \textbf{OPERAZIONE 2}: Calcolare le statistiche di superamento di un corso. Per ogni corso offerto dalla facoltà, restituire il numero totale di studenti che lo hanno superato e la media dei voti ottenuti.
    \item \textbf{OPERAZIONE 3}: Modifica del piano di studi. Inserimento o rimozione di un corso all'interno del piano di studi di uno studente. È un'operazione interattiva tipica dell'inizio dell'anno accademico.
    \item \textbf{OPERAZIONE 4}: Registrazione di un esame superato. Registrare un nuovo esame sostenuto da uno studente, inserendo la data, la tipologia, l'esito e segnandolo come "superato". Operazione di scrittura più utilizzata durante le sessioni d'esame.
    \item \textbf{OPERAZIONE 5}: Calcolo dei crediti acquisiti dallo studente. Dato un numero di matricola, restituire la somma totale dei crediti dei corsi per i quali lo studente ha sostenuto e superato l'esame. Operazione usata continuamente dagli studenti per controllare il proprio libretto.
\end{itemize}