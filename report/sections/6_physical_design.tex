...
Il secondo grande beneficio riguarda la coerenza dei dati, poiché la propagazione di una chiave composta crea un vincolo automatico a livello strutturale che impedisce a monte qualsiasi incongruenza temporale.
...
Alcuni vincoli di integrità non possono essere espressi mediante i tradizionali vincoli dichiarativi, poiché richiedono verifiche che coinvolgono più tabelle. Per questo motivo sono stati implementati trigger dedicati associati a funzioni scritte in linguaggio \texttt{PL/pgSQL}. Nel seguito vengono illustrati quelli maggiormente rappresentativi, mentre l'implementazione completa è disponibile nella repository GitHub del progetto all'indirizzo \url{https://github.com/cossidente/database-lab-project/blob/main/sql/02_triggers.sql}.

\paragraph{Controllo delle propedeuticità d'esame}
Prima di consentire l'inserimento di un nuovo record nella tabella \texttt{esame}, il sistema deve verificare lo stato della carriera dello studente. La funzione \texttt{controlla\_prerequisiti} intercetta l'operazione in modalità \texttt{BEFORE INSERT} e verifica se lo studente ha superato tutte le materie definite come propedeutiche per l'insegnamento in esame. In caso di esito negativo, l'operazione viene interrotta sollevando un'eccezione che elenca esplicitamente i codici dei corsi mancanti.
