...
Specularmente, per garantire la consistenza del saldo anche a fronte di rimozioni di record inseriti per errore o rettifiche, è stata implementata una logica di decremento speculare.
Per evitare conflitti con l'operazione di eliminazione a cascata (\texttt{ON DELETE CASCADE}) definita sulla chiave esterna verso la tabella \texttt{studente}, si è scelto di non bloccare la \texttt{DELETE} a monte.
Al contrario, la funzione \texttt{aggiorna\_crediti\_post\_delete()} agisce in modalità \texttt{AFTER DELETE} aggiornando il saldo solo se l'entità padre è ancora presente nel sistema.
Inoltre, grazie alla clausola \texttt{WHEN} applicata direttamente al trigger, il decremento viene eseguito esclusivamente se l'esame rimosso presentava un esito sufficiente ($\ge 18$), prevenendo la sottrazione di crediti mai acquisiti.

L'implementazione fisica della procedura e del relativo trigger è stata strutturata come segue:
...
Eventuali rettifiche non vengono gestite tramite modifiche dirette, ma devono essere effettuate attraverso l'eliminazione del record errato e la successiva reinserzione con i dati corretti.
