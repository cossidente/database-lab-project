...
A completamento dell'analisi sulle chiavi, sono stati introdotti ulteriori indici secondari (definiti nel file \texttt{sql/04\_indexes.sql}) per ottimizzare le operazioni più frequenti, distinguendo due categorie di necessità.

La prima categoria riguarda i filtri di ricerca utilizzati dalle interrogazioni presentate nel Capitolo~\ref{chap:operazioni-e-interrogazioni}. In particolare, \texttt{idx\_docente\_cognome\_nome} accelera la ricerca di un docente per nome e cognome (operazione di Sezione~\ref{ssec:estrazione-del-calendario-settimanale-di-un-docente}), \texttt{idx\_insegnamento\_nome} velocizza il filtro per nome di un insegnamento (operazione di Sezione~\ref{ssec:media-dei-tentativi-per-gli-studenti-promossi}), mentre \texttt{idx\_insegnamento\_edizione\_ricerca} supporta la giunzione tra \texttt{lezione} ed \texttt{edizione} tramite la tabella \texttt{insegnamento\_edizione}, sfruttata nella medesima operazione.

La seconda categoria riguarda invece le foreign key che corrispondono alla seconda colonna di una chiave primaria composta, e che quindi non beneficiano dell'indice automaticamente creato sulla chiave primaria. È il caso di \texttt{esame.codice\_insegnamento} e \texttt{piano\_di\_studio.codice\_insegnamento}, entrambe protette da un vincolo \texttt{ON DELETE RESTRICT} verso \texttt{insegnamento}: senza un indice dedicato, ogni cancellazione di un insegnamento richiederebbe una scansione completa di queste tabelle (rispettivamente 320\,000 e 320\,000 tuple secondo la Tabella~\ref{tab:volumi}) per verificarne l'assenza di riferimenti. Gli indici \texttt{idx\_esame\_insegnamento} e \texttt{idx\_piano\_di\_studio\_insegnamento} riducono tale controllo a un accesso diretto.
