...
L'operazione esegue inizialmente un raggruppamento per calcolare le occorrenze di ogni insegnamento tramite una \textit{Common Table Expression} (\texttt{CTE}) denominata \texttt{conteggi\_insegnamenti}.
...
\subsection{Sostituzione di una cattedra}
\label{ssec:sostituzione-di-una-cattedra}
In caso di necessità organizzative, la segreteria dell'ateneo deve poter trasferire istantaneamente la titolarità di un'edizione di un insegnamento da un docente a un nuovo sostituto.
...
Per garantire che l'operazione di \texttt{UPDATE} non assegni cattedre in modo illegittimo o a docenti non qualificati, è stata inserita una condizione di validazione tramite la sottoquery \texttt{EXISTS}.
...
Inoltre, l'anno accademico dell'edizione da aggiornare viene calcolato dinamicamente: questo vincola la sostituzione all'anno in corso, impedendo la modifica retroattiva della titolarità di cattedre passate.

\section{Implementazione delle operazioni di inserimento}
\label{sec:implementazione-delle-operazioni-di-inserimento}

Le operazioni di inserimento popolano e ampliano lo stato della base di dati, introducendo in modo permanente nuovi dati.
...
\label{ssec:inserimento-di-un-nuovo-insegnamento-e-assegnazione-della-cattedra}
...
Per garantire che gli inserimenti non generino stati inconsistenti o tuple orfane all'interno della base di dati, l'intera sequenza operativa è incapsulata all'interno di un blocco transazionale tramite le direttive \texttt{BEGIN} e \texttt{COMMIT}.
Il DBMS applicherà le modifiche seguendo un ordine gerarchico strettamente dettato dai vincoli di integrità referenziale: la creazione del record primario in \texttt{insegnamento} risulta infatti propedeutica all'apertura dell'\texttt{edizione} e alla registrazione in \texttt{abilitazione\_docente\_insegnamento}, condizioni a loro volta necessarie per legittimare l'inserimento conclusivo nella tabella \texttt{insegnamento\_edizione}. Qualsiasi violazione dei vincoli in una di queste fasi provocherebbe l'annullamento dell'intera transazione, preservando in modo assoluto la correttezza strutturale dello schema.