...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION
            'Il nuovo docente non possiede l'abilitazione per tutte le cattedre del docente da sostituire.';
    END IF;
END $$;

-- Se il controllo viene superato, aggiorna tutte le cattedre.
...
La sostituzione viene eseguita all'interno di una transazione. Prima dell'aggiornamento, un blocco procedurale \texttt{DO} verifica che il docente sostituto sia abilitato a tutti gli insegnamenti assegnati al docente uscente nell'anno accademico considerato. Se esiste anche una sola edizione per la quale il nuovo docente non possiede la relativa abilitazione, viene sollevata un'eccezione mediante \texttt{RAISE EXCEPTION}, interrompendo la transazione. Solo se il controllo viene superato viene eseguito l'\texttt{UPDATE} massivo che trasferisce tutte le cattedre al nuovo docente. In questo modo è garantita l'atomicità dell'operazione: il trasferimento avviene integralmente oppure non viene effettuata alcuna modifica al database.
