...
Questa prima query risponde all'esigenza di determinare quale insegnamento compaia più frequentemente all'interno dei piani di studio degli iscritti.
...
L'operazione esegue un raggruppamento sul codice dell'insegnamento e ordina i risultati in modo decrescente.
La combinazione della clausola \texttt{ORDER BY} con \texttt{LIMIT 1} permette di isolare immediatamente e in modo efficiente il singolo record con il conteggio massimo, senza dover ricorrere all'uso più dispendioso di sottoquery annidate.
...
La query quantifica la percentuale di prove d'esame superate con successo rispetto al volume totale dei tentativi registrati per un determinato insegnamento.
